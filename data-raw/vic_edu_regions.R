...
area.text =
  ...
  dplyr::filter(!stringr::str_detect(area_name,"Offices"))
...
region_tbl =
  ...
  dplyr::mutate(
   region_name = stringr::str_extract(value, ".*Victoria")
   ) |>
  tidyr::fill(region_name) |>
  dplyr::filter(!stringr::str_detect(value,region_name)) |>
  # Then get LGA names, which are lists with commas
  dplyr::mutate(
   lga_name = stringr::str_extract(value,".*,.*"),
   lga_name = stringr::str_remove_all(lga_name,"Offices")) |>
  # Then get area names
  dplyr::mutate(
   area_name = stringr::str_extract(lga_name,stringr::str_flatten(area.text$area_name,"|")),
   area_name = dplyr::coalesce(area_name,value)
   ) |>
  # Tidy data
  dplyr::filter(area_name!=lga_name|is.na(lga_name)) |>
  dplyr::mutate(area_name = dplyr::lag(area_name))  |>
  dplyr::filter(!is.na(lga_name)) |>
  dplyr::select(contains("name")) |>
  dplyr::mutate_all(\(x) stringr::str_remove_all(x,"\t")) |>
  dplyr::mutate(lga_name = stringr::str_split(lga_name,", ")) |>
  tidyr::unnest(lga_name) |>
  dplyr::mutate(lga_name=  stringr::str_remove(lga_name, stringr::str_flatten(area.text$area_name,"|"))) |>
...
lga_map =
  ...
  dplyr::filter(
    state_code == 2) |>
  dplyr::mutate(
    lga_name = stringr::str_remove(lga_name,"\\s\\(.*\\)"),
    lga_name = stringr::str_replace(lga_name,"Moreland","Merri-bek"))
